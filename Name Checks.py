...
cwd = os.getcwd()
batter_files = open(os.path.join(cwd, "2019 Batter Similarity Scores.csv"))
...
pitcher_file = open(os.path.join(cwd, "2019 Pitcher Similarity Scores.csv"))
names = pitcher_file.readlines()[0].split(",")[1:]
for file in os.listdir(os.path.join(cwd, "Bullpens")):
    if file.endswith("Bullpen.csv") and file.startswith("2019"):
